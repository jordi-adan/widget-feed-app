export class PrdWidgetType {
  ...
  public static create(value: string): PrdWidgetType {
    return new PrdWidgetType(value);
  }
  ...
  public equals(other: PrdWidgetType): boolean {
  ...
  private ensureValidType(value: string): void {
    if (!value || value.trim().length === 0) {
      throw new Error('PrdWidgetType cannot be empty');
    }

    if (!PrdWidgetType.VALID_TYPES.includes(value as any)) {
      throw new Error(
        `Invalid PRD widget type: ${value}. Valid types are: ${PrdWidgetType.VALID_TYPES.join(', ')}`
      );
    }
  }
}
